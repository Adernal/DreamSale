            ...
            for (int i = 0; i < attributes.Count; i++)
            {
                ...
                for (int j = 0; j < valuesStr.Count; j++)
                {
                    ...
                    if (!attribute.ShouldHaveValues())
                    {
                        ...
                        {
                            //multiline textbox
                            var attributeName = attribute.GetLocalized(a => a.Name, _workContext.WorkingLanguage.Id, _languageService, _localizedEntityService);
                            //encode (if required)
                            if (htmlEncode)
                        ...
                        else
                        {
                            ...
                            formattedAttribute = string.Format("{0}: {1}", attribute.GetLocalized(a => a.Name, _workContext.WorkingLanguage.Id, _languageService, _localizedEntityService), valueStr);
                            //encode (if required)
                            if (htmlEncode)
                    ...
                    else
                    {
                        ...
                        if (int.TryParse(valueStr, out attributeValueId))
                        {
                            ...
                            {
                                formattedAttribute = string.Format("{0}: {1}", attribute.GetLocalized(a => a.Name, _workContext.WorkingLanguage.Id, _languageService, _localizedEntityService), attributeValue.GetLocalized(a => a.Name, _workContext.WorkingLanguage.Id, _languageService, _localizedEntityService));
                                if (renderPrices)
                                {
