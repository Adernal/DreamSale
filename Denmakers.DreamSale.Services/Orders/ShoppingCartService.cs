...
namespace Denmakers.DreamSale.Services.Orders
{
    public partial class ShoppingCartService : IShoppingCartService
    {
            ...
            bool ensureOnlyActiveCheckoutAttributes = false)
        {
                ...
                shoppingCartItem.ShoppingCartType == ShoppingCartType.ShoppingCart)
            {
                var cart = customer.ShoppingCartItems
                    ...
                    .LimitPerStore(storeId, _shoppingCartSettings)
                    .ToList();
                        ...
                        if (product.AutomaticallyAddRequiredProducts)
                        {
                            ...
                            if (automaticallyAddRequiredProductsIfEnabled)
                            {
                                ...
                                if (addToCartWarnings.Any())
                                {
                                    ...
                                    warnings.Add(string.Format(_localizationService.GetResource("ShoppingCart.RequiredProductWarning"), rp.GetLocalized(x => x.Name, _workContext, _languageService, _localizedEntityService)));
                                }
                            }
                            else
                            {
                                    ...
                                    if (maximumQuantityCanBeAdded <= 0)
                                    {
                                        ...
                                        var warning = productAvailabilityRange == null ? _localizationService.GetResource("ShoppingCart.OutOfStock")
                                            : string.Format(_localizationService.GetResource("ShoppingCart.AvailabilityRange"),
                                                productAvailabilityRange.GetLocalized(range => range.Name, _workContext, _languageService, _localizedEntityService));
                ...
                if (a2.IsRequired)
                {
                    ...
                    if (!found)
                    {
                        var notFoundWarning = !string.IsNullOrEmpty(a2.TextPrompt) ?
                            a2.TextPrompt :
                            string.Format(_localizationService.GetResource("ShoppingCart.SelectAttribute"), a2.ProductAttribute.GetLocalized(a => a.Name, _workContext, _languageService, _localizedEntityService));
            ...
            foreach (var pam in attributes2)
            {
                ...
                if (pam.ValidationMinLength.HasValue)
                {
                        ...
                        pam.AttributeControlType == AttributeControlType.MultilineTextbox)
                    {
                        ...
                        {
                            warnings.Add(string.Format(_localizationService.GetResource("ShoppingCart.TextboxMinimumLength"), pam.ProductAttribute.GetLocalized(a => a.Name, _workContext, _languageService, _localizedEntityService), pam.ValidationMinLength.Value));
                        }
                    }
                }
                ...
                if (pam.ValidationMaxLength.HasValue)
                {
                        ...
                        pam.AttributeControlType == AttributeControlType.MultilineTextbox)
                    {
                        ...
                        {
                            warnings.Add(string.Format(_localizationService.GetResource("ShoppingCart.TextboxMaximumLength"), pam.ProductAttribute.GetLocalized(a => a.Name, _workContext, _languageService, _localizedEntityService), pam.ValidationMaxLength.Value));
                        }
                    }
                }
            }
            ...
            foreach (var attributeValue in attributeValues)
            {
                if (attributeValue.AttributeValueType == AttributeValueType.AssociatedToProduct)
                {
                    ...
                    if (associatedProduct != null)
                    {
                        ...
                        {
                            var attributeName = attributeValue.ProductAttributeMapping.ProductAttribute.GetLocalized(a => a.Name, _workContext, _languageService, _localizedEntityService);
                            var attributeValueName = attributeValue.GetLocalized(a => a.Name, _workContext, _languageService, _localizedEntityService);
            ...
            if (validateCheckoutAttributes)
            {
                ...
                foreach (var a2 in attributes2)
                {
                    if (a2.IsRequired)
                    {
                        ...
                        if (!found)
                        {
                            if (!string.IsNullOrEmpty(a2.GetLocalized(a => a.TextPrompt, _workContext, _languageService, _localizedEntityService)))
                                warnings.Add(a2.GetLocalized(a => a.TextPrompt, _workContext, _languageService, _localizedEntityService));
                            else
                                warnings.Add(string.Format(_localizationService.GetResource("ShoppingCart.SelectAttribute"), a2.GetLocalized(a => a.Name, _workContext, _languageService, _localizedEntityService)));
                        }
                    }
                }
                ...
                foreach (var ca in attributes2)
                {
                    if (ca.ValidationMinLength.HasValue)
                    {
                            ...
                            ca.AttributeControlType == AttributeControlType.MultilineTextbox)
                        {
                            ...
                            {
                                warnings.Add(string.Format(_localizationService.GetResource("ShoppingCart.TextboxMinimumLength"), ca.GetLocalized(a => a.Name, _workContext, _languageService, _localizedEntityService), ca.ValidationMinLength.Value));
                            }
                        }
                    }
                    ...
                    if (ca.ValidationMaxLength.HasValue)
                    {
                            ...
                            ca.AttributeControlType == AttributeControlType.MultilineTextbox)
                        {
                            ...
                            {
                                warnings.Add(string.Format(_localizationService.GetResource("ShoppingCart.TextboxMaximumLength"), ca.GetLocalized(a => a.Name, _workContext, _languageService, _localizedEntityService), ca.ValidationMaxLength.Value));
                            }
                        }
                    }
                }
        ...
        public virtual void MigrateShoppingCart(Customer fromCustomer, Customer toCustomer, bool includeCouponCodes)
        {
            ...
            {
                //discount
                foreach (var code in fromCustomer.ParseAppliedDiscountCouponCodes(_genericAttributeService))
                    toCustomer.ApplyDiscountCouponCode(code, _genericAttributeService);

                //gift card
                foreach (var code in fromCustomer.ParseAppliedGiftCardCouponCodes(_genericAttributeService))
                    toCustomer.ApplyGiftCardCouponCode(code, _genericAttributeService);
