...
namespace Denmakers.DreamSale.Services.Customers
{
    public static class CustomerExtensions
    {
        ...
        public static string GetFullName(this Customer customer, IGenericAttributeService genericAttributeService)
        {
            if (customer == null)
                throw new ArgumentNullException("customer");
            var firstName = customer.GetAttribute<string>(SystemCustomerAttributeNames.FirstName, genericAttributeService);
            var lastName = customer.GetAttribute<string>(SystemCustomerAttributeNames.LastName, genericAttributeService);
        ...
        public static bool IsPasswordRecoveryTokenValid(this Customer customer, string token, IGenericAttributeService genericAttributeService)
        {
            if (customer == null)
                throw new ArgumentNullException("customer");

            var cPrt = customer.GetAttribute<string>(SystemCustomerAttributeNames.PasswordRecoveryToken, genericAttributeService);
        ...
        /// <returns>Result</returns>
        public static bool IsPasswordRecoveryLinkExpired(this Customer customer, CustomerSettings customerSettings, IGenericAttributeService genericAttributeService)
        {
            if (customer == null)
                throw new ArgumentNullException("customer");
            ...
            var geneatedDate = customer.GetAttribute<DateTime?>(SystemCustomerAttributeNames.PasswordRecoveryTokenDateGenerated, genericAttributeService);
        ...
        public static bool PasswordIsExpired(this Customer customer, CustomerSettings customerSettings, ICustomerService customerService)
        {
            if (customer == null)
                throw new ArgumentNullException("customer");
            ...
            var customerPassword = customerService.GetCurrentPassword(customer.Id);
            //password is not found, so return max value to force customer to change password
            if (customerPassword == null)
                currentLifetime = int.MaxValue;
            else
                currentLifetime = (DateTime.UtcNow - customerPassword.CreatedOnUtc).Days;

            return currentLifetime >= customerSettings.PasswordLifetime;
        }
        #endregion

        #region Discount and Coupons
        /// <summary>
        /// Gets coupon codes
        /// </summary>
        /// <param name="customer">Customer</param>
        /// <returns>Coupon codes</returns>
        public static string[] ParseAppliedDiscountCouponCodes(this Customer customer, IGenericAttributeService genericAttributeService)
        {
            if (customer == null)
                throw new ArgumentNullException("customer");
        ...
        public static void ApplyDiscountCouponCode(this Customer customer, string couponCode, IGenericAttributeService genericAttributeService)
        {
            if (customer == null)
                throw new ArgumentNullException("customer");
        ...
        /// <returns>New coupon codes document</returns>
        public static void RemoveDiscountCouponCode(this Customer customer, string couponCode, IGenericAttributeService genericAttributeService)
        {
            if (customer == null)
                throw new ArgumentNullException("customer");

            //get applied coupon codes
            var existingCouponCodes = customer.ParseAppliedDiscountCouponCodes(genericAttributeService);
            ...
            foreach (string existingCouponCode in existingCouponCodes)
                if (!existingCouponCode.Equals(couponCode, StringComparison.InvariantCultureIgnoreCase))
                    customer.ApplyDiscountCouponCode(existingCouponCode, genericAttributeService);
        }
        ...
        /// <param name="customer">Customer</param>
        /// <returns>Coupon codes</returns>
        public static string[] ParseAppliedGiftCardCouponCodes(this Customer customer, IGenericAttributeService genericAttributeService)
        {
            if (customer == null)
                throw new ArgumentNullException("customer");
        ...
        /// <param name="couponCode">Coupon code</param>
        /// <returns>New coupon codes document</returns>
        public static void ApplyGiftCardCouponCode(this Customer customer, string couponCode, IGenericAttributeService genericAttributeService)
        {
            if (customer == null)
                throw new ArgumentNullException("customer");
        ...
        /// <param name="couponCode">Coupon code to remove</param>
        /// <returns>New coupon codes document</returns>
        public static void RemoveGiftCardCouponCode(this Customer customer, string couponCode, IGenericAttributeService genericAttributeService)
        {
            if (customer == null)
                throw new ArgumentNullException("customer");

            //get applied coupon codes
            var existingCouponCodes = customer.ParseAppliedGiftCardCouponCodes(genericAttributeService);
            ...
            foreach (string existingCouponCode in existingCouponCodes)
                if (!existingCouponCode.Equals(couponCode, StringComparison.InvariantCultureIgnoreCase))
                    customer.ApplyGiftCardCouponCode(existingCouponCode, genericAttributeService);
        }
        #endregion
    }
}
