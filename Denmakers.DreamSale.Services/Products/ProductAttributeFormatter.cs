                ...
                foreach (var attribute in _productAttributeParser.ParseProductAttributeMappings(attributesXml))
                {
                    ...
                    if (!attribute.ShouldHaveValues())
                    {
                        foreach (var value in _productAttributeParser.ParseValues(attributesXml, attribute.Id))
                        {
                            ...
                            {
                                //multiline textbox
                                var attributeName = attribute.ProductAttribute.GetLocalized(a => a.Name, _workContext.WorkingLanguage.Id, _languageService, _localizedEntityService);
                            ...
                            else
                            {
                                ...
                                formattedAttribute = string.Format("{0}: {1}", attribute.ProductAttribute.GetLocalized(a => a.Name, _workContext.WorkingLanguage.Id, _languageService, _localizedEntityService), value);
                    ...
                    else
                    {
                        foreach (var attributeValue in _productAttributeParser.ParseProductAttributeValues(attributesXml, attribute.Id))
                        {
                            var formattedAttribute = string.Format("{0}: {1}",
                                attribute.ProductAttribute.GetLocalized(a => a.Name, _workContext.WorkingLanguage.Id, _languageService, _localizedEntityService),
                                attributeValue.GetLocalized(a => a.Name, _workContext.WorkingLanguage.Id, _languageService, _localizedEntityService));
