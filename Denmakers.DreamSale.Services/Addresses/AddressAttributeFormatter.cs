...
namespace Denmakers.DreamSale.Services.Addresses
{
    public partial class AddressAttributeFormatter : IAddressAttributeFormatter
    {
        ...
        public AddressAttributeFormatter(IAddressAttributeService addressAttributeService, IAddressAttributeParser addressAttributeParser)
        {
            ...
            for (int i = 0; i < attributes.Count; i++)
            {
                ...
                for (int j = 0; j < valuesStr.Count; j++)
                {
                    ...
                    if (!attribute.ShouldHaveValues())
                    {
                        ...
                        {
                            //multiline textbox
                            var attributeName = attribute.Name;//.GetLocalized(a => a.Name, 1);
                            //encode (if required)
                            if (htmlEncode)
                        ...
                        else
                        {
                            ...
                            formattedAttribute = string.Format("{0}: {1}", attribute.Name/*.GetLocalized(a => a.Name, 1)*/, valueStr);
                            //encode (if required)
                            if (htmlEncode)
                    ...
                    else
                    {
                        ...
                        if (int.TryParse(valueStr, out attributeValueId))
                        {
                            ...
                            {
                                formattedAttribute = string.Format("{0}: {1}", attribute.Name/*.GetLocalized(a => a.Name, 1)*/, attributeValue.Name/*.GetLocalized(a => a.Name, 1)*/);
                            }
                            //encode (if required)
                            if (htmlEncode)
                                formattedAttribute = HttpUtility.HtmlEncode(formattedAttribute);
