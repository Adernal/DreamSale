...
using Denmakers.DreamSale.Services.eu.europa.ec;
...
namespace Denmakers.DreamSale.Services.Tax
{
    public partial class TaxService : ITaxService
    {
        ...
        protected virtual bool IsEuConsumer(Customer customer)
        {
            ...
            {
                var countryId = customer.GetAttribute<int>(SystemCustomerAttributeNames.CountryId, _genericAttributeService);
            ...
            var customerVatStatus = (VatNumberStatus)customer.GetAttribute<int>(SystemCustomerAttributeNames.VatNumberStatusId, _genericAttributeService);
        ...
        protected virtual CalculateTaxRequest CreateCalculateTaxRequest(Product product, int taxCategoryId, Customer customer, decimal price)
        {
            if (customer == null)
                throw new ArgumentNullException("customer");
            ...
            {
                var pickupPoint = customer.GetAttribute<PickupPoint>(SystemCustomerAttributeNames.SelectedPickupPoint, _genericAttributeService, _storeContext.CurrentStore.Id);
                if (pickupPoint != null)
                {
        ...
        public virtual decimal GetProductPrice(Product product, int taxCategoryId,
            decimal price, bool includingTax, Customer customer, bool priceIncludesTax, out decimal taxRate)
        {
        ...
        /// <param name="taxRate">Tax rate</param>
        /// <returns>Price</returns>
        public virtual decimal GetCheckoutAttributePrice(CheckoutAttributeValue cav, bool includingTax, Customer customer, out decimal taxRate)
        {
            if (cav == null)
