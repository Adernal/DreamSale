"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
var core_1 = require("@angular/core");
var common_1 = require("@angular/common");
var product_attributes_routing_module_1 = require("./product-attributes-routing.module");
var product_attributes_component_1 = require("./product-attributes.component");
var shared_1 = require("../../../shared");
var forms_1 = require("@angular/forms");
var http_1 = require("@angular/http");
var product_attributes_service_1 = require("./product-attributes.service");
var filter_pipe_1 = require("./filter.pipe");
var ProductAttributesModule = (function () {
    function ProductAttributesModule() {
    }
    return ProductAttributesModule;
}());
ProductAttributesModule = __decorate([
    core_1.NgModule({
        imports: [
            common_1.CommonModule,
            product_attributes_routing_module_1.ProductAttributesRoutingModule,
            shared_1.PageHeaderModule,
            forms_1.FormsModule,
            http_1.HttpModule
        ],
        providers: [product_attributes_service_1.ProductAttributesService],
        declarations: [product_attributes_component_1.ProductAttributesComponent, filter_pipe_1.FilterPipe]
    })
], ProductAttributesModule);
exports.ProductAttributesModule = ProductAttributesModule;
//# sourceMappingURL=product-attributes.module.js.map