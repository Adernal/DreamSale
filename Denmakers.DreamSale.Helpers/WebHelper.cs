...
namespace Denmakers.DreamSale.Helpers
{
    public partial class WebHelper : IWebHelper
    {
        ...
        public virtual string GetCurrentIpAddress()
        {
            ...
            {
                result = "127.0.0.1";
                // Get user's machine IP address instead of vendor's IP
                string hostName = System.Net.Dns.GetHostName(); // Retrive the Name of HOST
                var ipAddresses = System.Net.Dns.GetHostAddresses(hostName).ToList().Select(ip => Convert.ToString(ip));
                if (ipAddresses != null && ipAddresses.Any())
                {
                    result = Convert.ToString(ipAddresses.FirstOrDefault(ip => !ip.Contains("::")));
                }
            }
            //remove port
        ...
        public virtual void RestartAppDomain(bool makeRedirect = false, string redirectUrl = "")
        {
            ...
            else
            {
                ...
                if (!success)
                {
                    throw new DreamSaleException("DreamSale needs to be restarted due to a configuration change, but was unable to do so." + Environment.NewLine +
        ...
        public virtual bool IsPostBeingDone
        {
            get
            {
                if (_httpContext.Items["DreamSale.IsPOSTBeingDone"] == null)
                    return false;
                return Convert.ToBoolean(_httpContext.Items["DreamSale.IsPOSTBeingDone"]);
            }
            set
            {
                _httpContext.Items["DreamSale.IsPOSTBeingDone"] = value;
            }
        }
